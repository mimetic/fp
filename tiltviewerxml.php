...
Example:
	http://localhost/fp/slideshowxml.php?params=medium-pictures,action-project,id-13,maxImageWidth-480,maxImageHeight-480,textColor-0xFFFFFF,frameColor-0xffffff,frameWidth-20,stagePadding-40,thumbnailColumns-3,thumbnailRows-3,navPosition-left,enableRightClickOpen-true,backgroundImagePath-
...
function buildImage ($image) {
	...
	if ( $image['URL'] ) {
		...
		$caption .= " (photo by " . $image['Byline'] . ")";
		$caption = trim( str_replace ("\n", " ", str_replace ("\r", " ", $caption ) ) );
		$oneimage = array (			"filename"	=>	$image['URL'],
									"caption"	=>	$caption,
									"title"	=>	$image['Title'],
...
class SlideshowXML {
	...
	function addAlbum ($albumID, $params, $project, $images = array ()) {
		...
		$album = array ();
		$album['maxImageWidth'] =$this->params['maxImageWidth'];
		$album['maxImageHeight'] =$this->params['maxImageHeight'];
		$album['textColor'] =$this->params['textColor'];
		$album['frameColor'] =$this->params['frameColor'];
		$album['frameWidth'] =$this->params['frameWidth'];
		$album['stagePadding'] =$this->params['stagePadding'];
		$album['navPadding'] =$this->params['navPadding'];
		$album['thumbnailColumns'] =$this->params['thumbnailColumns'];
		$album['thumbnailRows'] =$this->params['thumbnailRows'];
		$album['vAlign'] =$this->params['vAlign'];
		$album['hAlign'] =$this->params['hAlign'];
		$album['navPosition'] =$this->params['navPosition'];
	...
	//	header("Content-type: application/XML+xml");
		...
		$DEBUG = 1;
		$DEBUG && file_put_contents ("gallery.xml", $output);
		...
		foreach ($this->albums as $album) {
			...
			foreach ($album['images'] as $image) {
				...
				$filename = "http://$fp/$SlidePath" . $image["filename"] ;
					...
					$output .=  "<photo imageurl=\"$filename\">\n";
					if ($this->params['showCaption'] == "title") {
						$output .=  "\t<title>" . htmlspecialchars($image["title"], ENT_QUOTES) . "</title>\n";
					} elseif ($this->params['showCaption'] != "off")  {
						$output .=  "\t<description>" . htmlspecialchars($image["caption"], ENT_QUOTES) . "</description>\n";
					}
					$output .=  "</photo>\n";
			}
