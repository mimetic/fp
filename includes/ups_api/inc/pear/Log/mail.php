...
class Log_mail extends Log {
	...
	function close() {
		if ($this->_opened) {
			if ($this->_shouldSend && ! empty ( $this->_message )) {
				...
				if (mail ( $this->_recipients, $this->_subject, $this->_message, $headers ) == false) {
	...
	function log($message, $priority = null) {
		...
		$this->_message .= $this->_format ( $this->_lineFormat, strftime ( $this->_timeFormat ), $priority, $message ) . "\r\n";
