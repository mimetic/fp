...
class PEAR_RunTest {
	var $_logger;
	var $_options;
	var $ini_overwrites = array ('output_handler=', 'open_basedir=', 'safe_mode=0', 'disable_functions=', 'output_buffering=Off', 'error_reporting=E_ALL', 'display_errors=1', 'log_errors=0', 'html_errors=0', 'track_errors=1', 'report_memleaks=1', 'report_zend_debug=0', 'docref_root=', 'docref_ext=.html', 'error_prepend_string=', 'error_append_string=', 'auto_prepend_file=', 'auto_append_file=', 'magic_quotes_runtime=0' );
	...
	function system_with_timeout($commandline, $env = null, $stdin = null) {
		...
		if (version_compare ( phpversion (), '5.0.0', '<' )) {
			$proc = proc_open ( $commandline, array (0 => array ('pipe', 'r' ), 1 => array ('pipe', 'w' ), 2 => array ('pipe', 'w' ) ), $pipes );
		} else {
	...
	function run($file, $ini_settings = '') {
		...
		$section_text = array ('TEST' => '(unnamed test)', 'SKIPIF' => '', 'GET' => '', 'COOKIE' => '', 'POST' => '', 'ARGS' => '', 'INI' => '', 'CLEAN' => '' );
		...
		if (! empty ( $section_text ['GET'] ) || ! empty ( $section_text ['POST'] ) || ! empty ( $section_text ['POST_RAW'] ) || ! empty ( $section_text ['COOKIE'] ) || ! empty ( $section_text ['UPLOAD'] )) {
		...
		if (array_key_exists ( 'UPLOAD', $section_text ) && ! empty ( $section_text ['UPLOAD'] )) {
			...
			foreach ( $upload_files as $fileinfo ) {
				...
				if (! realpath ( dirname ( $file ) . '/' . $fileinfo [1] )) {
					return PEAR::raiseError ( "File for upload does not exist: $fileinfo[1] " . "in test file: $file" );
				}
		...
		$headers = "";
		if (! empty ( $this->_options ['cgi'] ) && preg_match ( "/^(.*?)\r?\n\r?\n(.*)/s", $out, $match )) {
			$output = trim ( $match [2] );
			$rh = preg_split ( "/[\n\r]+/", $match [1] );
			$headers = array ( );
			foreach ( $rh as $line ) {
    ...
    */
		...
		if (array_key_exists ( 'FAIL', $section_text )) {
			...
			$faildiff = $this->generate_diff ( $wanted, $output, null, isset ( $section_text ['EXPECTF'] ) ? $wanted_re : null );
			...
			if (isset ( $section_text ['RETURNS'] )) {
				return PEAR::raiseError ( 'Cannot have both RETURNS and FAIL in the same test: ' . $file );
			}
		}
		...
		if (isset ( $section_text ['RETURNS'] )) {
			$GLOBALS ['__PHP_FAILED_TESTS__'] [] = array ('name' => $file, 'test_name' => $tested, 'output' => $log_filename, 'diff' => $diff_filename, 'info' => $info, 'return' => $return_value );
		} else {
			$GLOBALS ['__PHP_FAILED_TESTS__'] [] = array ('name' => $file, 'test_name' => $tested, 'output' => $output_filename, 'diff' => $diff_filename, 'info' => $info );
		}
		...
		if (strpos ( $log_format, 'D' ) !== FALSE) {
			...
			fwrite ( $log, $this->generate_diff ( $wanted, $output, isset ( $section_text ['RETURNS'] ) ? array (trim ( $section_text ['RETURNS'] ), $return_value ) : null, isset ( $section_text ['EXPECTF'] ) ? $wanted_re : null ) );
			fclose ( $log );
		}
...
" );
	...
	function generate_diff($wanted, $output, $return_value, $wanted_re) {
		...
		foreach ( $w1 as $idx => $val ) {
			if (! $wanted_re || ! isset ( $wr [$idx] ) || ! isset ( $o1 [$idx] ) || ! preg_match ( '/^' . $wr [$idx] . '$/', $o1 [$idx] )) {
				$w2 [sprintf ( "%03d<", $idx )] = sprintf ( "%03d- ", $idx + 1 ) . $val;
			}
		}
		foreach ( $o1 as $idx => $val ) {
			if (! $wanted_re || ! isset ( $wr [$idx] ) || ! preg_match ( '/^' . $wr [$idx] . '$/', $val )) {
