...
class PEAR_Downloader extends PEAR_Common {
			...
			if (PEAR::isError ( $err )) {
				...
				do {
					...
					if ($params [$i] && ! isset ( $channelschecked [$params [$i]->getChannel ()] ) && ! isset ( $this->_options ['offline'] )) {
						...
						$a = $this->downloadHttp ( 'http://' . $params [$i]->getChannel () . '/channel.xml', $this->ui, $dir, null, $curchannel->lastModified () );
	...
	function getDownloadDir() {
		...
		if (! is_writable ( $downloaddir )) {
			if (PEAR::isError ( System::mkdir ( array ('-p', $downloaddir ) ) )) {
				return PEAR::raiseError ( 'download directory "' . $downloaddir . '" is not writeable.  Change download_dir config variable to ' . 'a writeable dir' );
			}
		}
	...
	function _getPackageDownloadUrl($parr) {
		...
		if ($chan->supportsREST ( $this->config->get ( 'preferred_mirror' ) ) && $base = $chan->getBaseURL ( 'REST1.0', $this->config->get ( 'preferred_mirror' ) )) {
			...
			PEAR::staticPopErrorHandling ();
			if (! isset ( $this->_options ['force'] ) && ! isset ( $this->_options ['downloadonly'] ) && ! PEAR::isError ( $testversion ) && ! isset ( $parr ['group'] )) {
				if (version_compare ( $testversion, $url ['version'], '>=' )) {
					return PEAR::raiseError ( $this->_registry->parsedPackageNameToString ( $parr, true ) . ' is already installed and is newer than detected ' . 'release version ' . $url ['version'], - 976 );
				}
			}
	...
	function sortPkgDeps(&$packages, $uninstall = false) {
		$uninstall ? $this->sortPackagesForUninstall ( $packages ) : $this->sortPackagesForInstall ( $packages );
	}
	...
	function downloadHttp($url, &$ui, $save_dir = '.', $callback = null, $lastmodified = null, $accept = false) {
		...
		$request .= $ifmodifiedsince . "User-Agent: PEAR/1.5.1/PHP/" . PHP_VERSION . "\r\n";
