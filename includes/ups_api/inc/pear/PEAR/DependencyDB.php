...
class PEAR_DependencyDB {
	...
	function &singleton(&$config, $depdb = false) {
		if (! isset ( $GLOBALS ['_PEAR_DEPENDENCYDB_INSTANCE'] [$config->get ( 'php_dir', null, 'pear.php.net' )] )) {
			$a = new PEAR_DependencyDB ( );
			$GLOBALS ['_PEAR_DEPENDENCYDB_INSTANCE'] [$config->get ( 'php_dir', null, 'pear.php.net' )] = &$a;
			$a->setConfig ( $config, $depdb );
			if (PEAR::isError ( $e = $a->assertDepsDB () )) {
				return $e;
			}
		}
		return $GLOBALS ['_PEAR_DEPENDENCYDB_INSTANCE'] [$config->get ( 'php_dir', null, 'pear.php.net' )];
	}
	...
	function assertDepsDB() {
		if (! is_file ( $this->_depdb )) {
			...
			if ($depdb ['_version'] {0} > $this->_version {0}) {
				return PEAR::raiseError ( 'Dependency database is version ' . $depdb ['_version'] . ', and we are version ' . $this->_version . ', cannot continue' );
			}
		}
	}
	...
	function getDependentPackageDependencies(&$pkg) {
		...
		foreach ( $depend as $info ) {
			...
			foreach ( $temp as $dep ) {
				if (strtolower ( $dep ['dep'] ['channel'] ) == strtolower ( $channel ) && strtolower ( $dep ['dep'] ['name'] ) == strtolower ( $package )) {
	...
	function _dependsOn($parent, $child, &$checked) {
		...
		foreach ( $this->_cache ['dependencies'] [$channel] [$package] as $info ) {
			...
			if (strtolower ( $info ['dep'] ['channel'] ) == strtolower ( $depchannel ) && strtolower ( $info ['dep'] ['name'] ) == strtolower ( $deppackage )) {
				return true;
			}
		}
		foreach ( $this->_cache ['dependencies'] [$channel] [$package] as $info ) {
			if (isset ( $info ['dep'] ['uri'] )) {
				if ($this->_dependsOn ( array ('uri' => $info ['dep'] ['uri'], 'package' => $info ['dep'] ['name'] ), $child, $checked )) {
					return true;
				}
			} else {
				if ($this->_dependsOn ( array ('channel' => $info ['dep'] ['channel'], 'package' => $info ['dep'] ['name'] ), $child, $checked )) {
					return true;
				}
			}
		}
		...
		foreach ( $data ['dependencies'] [$channel] [$package] as $dep ) {
			...
			if ($found) {
				...
				if (! count ( $data ['packages'] [$depchannel] [strtolower ( $dep ['dep'] ['name'] )] )) {
					...
					$data ['packages'] [$depchannel] [strtolower ( $dep ['dep'] ['name'] )] = array_values ( $data ['packages'] [$depchannel] [strtolower ( $dep ['dep'] ['name'] )] );
				}
			}
		}
	...
	function _lock($mode = LOCK_EX) {
		if (! eregi ( 'Windows 9', php_uname () )) {
			...
			if (! is_resource ( $this->_lockFp )) {
				return PEAR::raiseError ( "could not create Dependency lock file" . (isset ( $php_errormsg ) ? ": " . $php_errormsg : "") );
			}
	...
	function _setPackageDeps(&$data, &$pkg) {
		...
		if ($data ['dependencies'] [strtolower ( $pkg->getChannel () )] [strtolower ( $pkg->getPackage () )] == array ( )) {
			unset ( $data ['dependencies'] [strtolower ( $pkg->getChannel () )] [strtolower ( $pkg->getPackage () )] );
	...
	function _registerDep(&$data, &$pkg, $dep, $type, $group = false) {
		$info = array ('dep' => $dep, 'type' => $type, 'group' => $group );
		...
		if (isset ( $data ['packages'] [strtolower ( $depchannel )] [strtolower ( $dep ['name'] )] )) {
			$found = false;
			foreach ( $data ['packages'] [strtolower ( $depchannel )] [strtolower ( $dep ['name'] )] as $i => $p ) {
				if ($p ['channel'] == strtolower ( $pkg->getChannel () ) && $p ['package'] == strtolower ( $pkg->getPackage () )) {
					$found = true;
					break;
				}
			}
			if (! $found) {
				$data ['packages'] [strtolower ( $depchannel )] [strtolower ( $dep ['name'] )] [] = array ('channel' => strtolower ( $pkg->getChannel () ), 'package' => strtolower ( $pkg->getPackage () ) );
			}
		} else {
