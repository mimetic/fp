...
class PEAR_Dependency {
	...
	function callCheckMethod(&$errmsg, $opts) {
		...
		$opt = (isset ( $opts ['optional'] ) && $opts ['optional'] == 'yes') ? $opts ['optional'] : null;
		$errmsg = '';
		switch ( $opts ['type']) {
			case 'pkg' :
		...
		switch ( $relation) {
			...
			case 'gt' :
				...
				if (! $this->registry->packageExists ( $name, $channel ) || ! version_compare ( "$version", "$req", $relation )) {
					...
					if ($opt) {
						$errmsg = "package `$channel/$name' version " . $this->signOperator ( $relation ) . " $req is recommended to utilize some features.";
						if ($version) {
							$errmsg .= "  Installed version is $version";
						}
						return $code;
					}
					$errmsg = "requires package `$channel/$name' " . $this->signOperator ( $relation ) . " $req";
					return $code;
				}
				return false;
		}
		...
		foreach ( $channels as $channelname => $packages ) {
			foreach ( $packages as $pkg ) {
				...
				foreach ( $deps as $dep ) {
					...
					if ($dep ['type'] == 'pkg' && (strcasecmp ( $dep ['name'], $package ) == 0) && ($depchannel == $channel)) {
						if ($dep ['rel'] == 'ne') {
							continue;
						}
	...
	function checkExtension(&$errmsg, $name, $req = null, $relation = 'has', $opt = false) {
		...
		if (! version_compare ( "$ext_ver", "$req", $operator )) {
			$errmsg = "'$name' PHP extension version " . $this->signOperator ( $operator ) . " $req is required";
			$code = $this->codeFromRelation ( $relation, $ext_ver, $req, $opt );
			if ($opt) {
				$errmsg = "'$name' PHP extension version " . $this->signOperator ( $operator ) . " $req is recommended to utilize some features";
				return $code;
			}
		}
		return $code;
	...
	function checkPHP(&$errmsg, $req, $relation = 'ge') {
		...
		if (! version_compare ( "$php_ver", "$req", $operator )) {
			$errmsg = "PHP version " . $this->signOperator ( $operator ) . " $req is required";
			return PEAR_DEPENDENCY_CONFLICT;
		}
		return false;
	}
	...
	function checkZend(&$errmsg, $req, $relation = 'ge') {
		...
		if (! version_compare ( "$zend_ver", "$req", $operator )) {
			$errmsg = "Zend version " . $this->signOperator ( $operator ) . " $req is required";
			return PEAR_DEPENDENCY_CONFLICT;
		}
		return false;
	}
		...
		switch ( $relation) {
			...
			case 'eq' :
				...
				if ($need_major > $have_major) {
					$code = $opt ? PEAR_DEPENDENCY_UPGRADE_MAJOR_OPTIONAL : PEAR_DEPENDENCY_UPGRADE_MAJOR;
				} else {
					$code = $opt ? PEAR_DEPENDENCY_UPGRADE_MINOR_OPTIONAL : PEAR_DEPENDENCY_UPGRADE_MINOR;
				}
			break;
			case 'lt' :
			case 'le' :
			case 'ne' :
				$code = $opt ? PEAR_DEPENDENCY_CONFLICT_OPTIONAL : PEAR_DEPENDENCY_CONFLICT;
			break;
		}
		return $code;
	}
