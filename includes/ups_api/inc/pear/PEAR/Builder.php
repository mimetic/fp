...
class PEAR_Builder extends PEAR_Common {
	...
	function _build_win32($descfile, $callback = null) {
		...
		$built_files [] = array ('file' => "$outfile", 'php_api' => $this->php_api_version, 'zend_mod_api' => $this->zend_module_api_no, 'zend_ext_api' => $this->zend_extension_api_no );
	...
	function _harvestInstDir($dest_prefix, $dirname, &$built_files) {
		...
		while ( ($ent = readdir ( $d )) !== false ) {
			...
			if (is_dir ( $full )) {
				...
				$built_files [] = array ('file' => $full, 'dest' => $dest, 'php_api' => $this->php_api_version, 'zend_mod_api' => $this->zend_module_api_no, 'zend_ext_api' => $this->zend_extension_api_no );
			}
		}
		closedir ( $d );
		return $ret;
