...
class PEAR_PackageFile_Generator_v1 {
	 ...
	 */
	function toPackageFile($where = null, $state = PEAR_VALIDATE_NORMAL, $name = 'package.xml', $nofilechecking = false) {
		if (! $this->_packagefile->validate ( $state, $nofilechecking )) {
			return PEAR::raiseError ( 'PEAR_Packagefile_v1::toPackageFile: invalid package.xml', null, null, null, $this->_packagefile->getValidationWarnings () );
		}
		if ($where === null) {
			if (! ($where = System::mktemp ( array ('-d' ) ))) {
			...
			return PEAR::raiseError ( 'PEAR_Packagefile_v1::toPackageFile: "' . $where . '" could' . ' not be created' );
		}
		$newpkgfile = $where . DIRECTORY_SEPARATOR . $name;
		$np = @fopen ( $newpkgfile, 'wb' );
		if (! $np) {
			return PEAR::raiseError ( 'PEAR_Packagefile_v1::toPackageFile: unable to save ' . "$name as $newpkgfile" );
		}
	...
	function _makeReleaseXml($pkginfo, $changelog = false, $state = PEAR_VALIDATE_NORMAL) {
		...
		if (isset ( $pkginfo ['configure_options'] )) {
			...
			foreach ( $pkginfo ['configure_options'] as $c ) {
				$ret .= "$indent   <configureoption name=\"" . $this->_fixXmlEncoding ( $c ['name'] ) . "\"";
				if (isset ( $c ['default'] )) {
			...
			if ($state ^ PEAR_VALIDATE_PACKAGING) {
				...
				foreach ( $pkginfo ['filelist'] as $file => $fa ) {
					...
					if (isset ( $fa ['baseinstalldir'] )) {
						$ret .= ' baseinstalldir="' . $this->_fixXmlEncoding ( $fa ['baseinstalldir'] ) . '"';
					}
					if (isset ( $fa ['md5sum'] )) {
					...
					if (! empty ( $fa ['install-as'] )) {
						$ret .= ' install-as="' . $this->_fixXmlEncoding ( $fa ['install-as'] ) . '"';
					}
	...
	function _formatFile($file, $attributes, $indent) {
		...
		if (isset ( $attributes ['baseinstalldir'] )) {
			$ret .= ' baseinstalldir="' . $this->_fixXmlEncoding ( $attributes ['baseinstalldir'] ) . '"';
		}
		...
		if (! empty ( $attributes ['install-as'] )) {
			$ret .= ' install-as="' . $this->_fixXmlEncoding ( $attributes ['install-as'] ) . '"';
		}
		$ret .= ' name="' . $this->_fixXmlEncoding ( $file ) . '"';
	...
	function _convertDependencies2_0(&$release, $internal = false) {
		$peardep = array ('pearinstaller' => array ('min' => '1.4.0b1' ) ); // this is a lot safer
	...
	function _convertFilelist2_0(&$package) {
		$ret = array ('dir' => array ('attribs' => array ('name' => '/' ), 'file' => array ( ) ) );
		$package ['platform'] = $package ['install-as'] = array ( );
	...
	function _convertRelease2_0(&$release, $package) {
		...
		if (count ( $package ['platform'] ) || count ( $package ['install-as'] )) {
			...
			foreach ( $package ['install-as'] as $file => $as ) {
				...
				//o <install as=..> tags for <file name=... platform=!... install-as=..>
				if (isset ( $package ['platform'] [$file] ) && $package ['platform'] [$file] {0} == '!') {
					$generic [] = $file;
					continue;
				}
				//o <ignore> tags for <file name=... platform=... install-as=..>
			...
			if (count ( $package ['platform'] )) {
				...
				foreach ( $oses as $os => $releaseNum ) {
					...
					$release [$releaseNum] ['filelist'] = array ('install' => array ( ), 'ignore' => array ( ) );
					foreach ( $package ['install-as'] as $file => $as ) {
						//o <install as=..> tags for <file name=... install-as=...>
						if (! isset ( $package ['platform'] [$file] )) {
							$release [$releaseNum] ['filelist'] ['install'] [] = array ('attribs' => array ('name' => $file, 'as' => $as ) );
							continue;
						}
						//o <install as..> tags for
						//  <file name=... platform=this platform install-as=..>
						if (isset ( $package ['platform'] [$file] ) && $package ['platform'] [$file] == $os) {
							$release [$releaseNum] ['filelist'] ['install'] [] = array ('attribs' => array ('name' => $file, 'as' => $as ) );
							continue;
						}
						//o <install as..> tags for
						//  <file name=... platform=!other platform install-as=..>
						if (isset ( $package ['platform'] [$file] ) && $package ['platform'] [$file] != "!$os" && $package ['platform'] [$file] {0} == '!') {
							$release [$releaseNum] ['filelist'] ['install'] [] = array ('attribs' => array ('name' => $file, 'as' => $as ) );
							continue;
						}
						//o <ignore> tags for
						//  <file name=... platform=!this platform install-as=..>
						if (isset ( $package ['platform'] [$file] ) && $package ['platform'] [$file] == "!$os") {
							$release [$releaseNum] ['filelist'] ['ignore'] [] = array ('attribs' => array ('name' => $file ) );
							continue;
						}
						//o <ignore> tags for
						//  <file name=... platform=other platform install-as=..>
						if (isset ( $package ['platform'] [$file] ) && $package ['platform'] [$file] {0} != '!' && $package ['platform'] [$file] != $os) {
							$release [$releaseNum] ['filelist'] ['ignore'] [] = array ('attribs' => array ('name' => $file ) );
							continue;
						}
					}
				...
				if (count ( $generic ) || count ( $genericIgnore )) {
					...
					if (count ( $generic )) {
						foreach ( $generic as $file ) {
							...
							$release [count ( $oses )] ['filelist'] ['install'] [] = array ('attribs' => array ('name' => $file, 'as' => $installas ) );
						}
					}
					if (count ( $genericIgnore )) {
						foreach ( $genericIgnore as $file ) {
							$release [count ( $oses )] ['filelist'] ['ignore'] [] = array ('attribs' => array ('name' => $file ) );
						}
					}
				}
				// cleanup
				foreach ( $release as $i => $rel ) {
					if (isset ( $rel ['filelist'] ['install'] ) && count ( $rel ['filelist'] ['install'] ) == 1) {
						$release [$i] ['filelist'] ['install'] = $release [$i] ['filelist'] ['install'] [0];
					}
					if (isset ( $rel ['filelist'] ['ignore'] ) && count ( $rel ['filelist'] ['ignore'] ) == 1) {
						$release [$i] ['filelist'] ['ignore'] = $release [$i] ['filelist'] ['ignore'] [0];
					}
				}
				if (count ( $release ) == 1) {
					$release = $release [0];
				...
				foreach ( $package ['install-as'] as $file => $value ) {
					$release ['filelist'] ['install'] [] = array ('attribs' => array ('name' => $file, 'as' => $value ) );
				}
