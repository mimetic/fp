...
class PEAR_Command_Channels extends PEAR_Command_Common {
	...
	var $commands = array ('list-channels' => array ('summary' => 'List Available Channels', 'function' => 'doList', 'shortcut' => 'lc', 'options' => array ( ), 'doc' => '
...
' ) );
	...
	function doList($command, $options, $params) {
		...
		$i = $j = 0;
		$data = array ('caption' => 'Registered Channels:', 'border' => true, 'headline' => array ('Channel', 'Summary' ) );
		foreach ( $registered as $channel ) {
			$data ['data'] [] = array ($channel->getName (), $channel->getSummary () );
		}
	...
	function doUpdateAll($command, $options, $params) {
		...
		foreach ( $channels as $channel ) {
			...
			if ($reg->channelExists ( $channel, true )) {
				...
				$contents = $dl->downloadHttp ( 'http://' . $test->getName () . '/channel.xml', $this->ui, $tmpdir, null, $lastmodified );
				PEAR::staticPopErrorHandling ();
				if (PEAR::isError ( $contents )) {
					$this->ui->outputData ( 'ERROR: Cannot retrieve channel.xml for channel "' . $test->getName () . '"', $command );
					continue;
				}
				if (! $contents) {
				...
				if ($channelinfo->getErrors ()) {
					$this->ui->outputData ( "Downloaded channel data from channel \"$channel\" " . 'is corrupt, skipping', $command );
					continue;
				}
				$channel = $channelinfo;
				if ($channel->getName () != $save) {
					$this->ui->outputData ( 'ERROR: Security risk - downloaded channel ' . 'definition file for channel "' . $channel->getName () . ' from channel "' . $save . '".  To use anyway, use channel-update', $command );
					continue;
				}
				...
				if ($reg->isAlias ( $channel )) {
					...
					if ($reg->channelExists ( $temp->getName () )) {
						$this->ui->outputData ( 'ERROR: existing channel "' . $temp->getName () . '" is aliased to "' . $channel . '" already and cannot be ' . 're-aliased to "' . $temp->getName () . '" because a channel with ' . 'that name or alias already exists!  Please re-alias and try ' . 'again.', $command );
						continue;
					}
				}
				$this->ui->outputData ( "Adding new channel \"$channel\"", $command );
				PEAR::staticPushErrorHandling ( PEAR_ERROR_RETURN );
				$contents = $dl->downloadHttp ( 'http://' . $channel . '/channel.xml', $this->ui, $tmpdir, null, false );
				PEAR::staticPopErrorHandling ();
				if (PEAR::isError ( $contents )) {
					$this->ui->outputData ( 'ERROR: Cannot retrieve channel.xml for channel "' . $channel . '"', $command );
					continue;
				}
				list ( $contents, $lastmodified ) = $contents;
				$info = implode ( '', file ( $contents ) );
	...
	function doAdd($command, $options, $params) {
		...
		if ($reg->channelExists ( $channel->getName () )) {
			return $this->raiseError ( 'channel-add: Channel "' . $channel->getName () . '" exists, use channel-update to update entry' );
		}
	...
	function doUpdate($command, $options, $params) {
		...
		if ((! file_exists ( $params [0] ) || is_dir ( $params [0] )) && $reg->channelExists ( strtolower ( $params [0] ) )) {
			...
			PEAR::staticPopErrorHandling ();
			if (PEAR::isError ( $contents )) {
				return $this->raiseError ( 'Cannot retrieve channel.xml for channel "' . $c->getName () . '"' );
			}
			list ( $contents, $lastmodified ) = $contents;
			if (! $contents) {
	...
	function doDiscover($command, $options, $params) {
		...
		if ($reg->channelExists ( $params [0] )) {
			if ($reg->isAlias ( $params [0] )) {
				return $this->raiseError ( "A channel alias named \"$params[0]\" " . 'already exists, aliasing channel "' . $reg->channelName ( $params [0] ) . '"' );
			} else {
		...
		if (PEAR::isError ( $err )) {
			return $this->raiseError ( "Discovery of channel \"$params[0]\" failed (" . $err->getMessage () . ')' );
		}
