...
class PEAR_Command_Registry extends PEAR_Command_Common {
	...
	var $commands = array ('list' => array ('summary' => 'List Installed Packages In The Default Channel', 'function' => 'doList', 'shortcut' => 'l', 'options' => array ('channel' => array ('shortopt' => 'c', 'doc' => 'list installed packages from this channel', 'arg' => 'CHAN' ), 'allchannels' => array ('shortopt' => 'a', 'doc' => 'list installed packages from all channels' ) ), 'doc' => '<package>
...
installed package.' ) );
	...
	function doList($command, $options, $params) {
		...
		$i = $j = 0;
		$data = array ('caption' => 'Installed packages, channel ' . $channel . ':', 'border' => true, 'headline' => array ('Package', 'Version', 'State' ) );
		foreach ( $installed as $package ) {
			$pobj = $reg->getPackage ( isset ( $package ['package'] ) ? $package ['package'] : $package ['name'], $channel );
			$data ['data'] [] = array ($pobj->getPackage (), $pobj->getVersion (), $pobj->getState () ? $pobj->getState () : null );
		}
	...
	function doListAll($command, $options, $params) {
		...
		foreach ( $installed as $channel => $packages ) {
			...
			if (count ( $packages ) == 0) {
				$data = array ('caption' => 'Installed packages, channel ' . $channel . ':', 'border' => true, 'data' => array (array ('(no packages installed)' ) ) );
			}
			$this->ui->outputData ( $data, $command );
		}
		return true;
		...
		if ($info->getPackagexmlVersion () == '1.0' || $installed) {
			foreach ( $list as $file => $att ) {
				if ($installed) {
					...
					if (isset ( $att ['baseinstalldir'] ) && ! in_array ( $att ['role'], array ('test', 'data', 'doc' ) )) {
					...
					switch ( $att ['role']) {
						...
						case 'doc' :
							...
							$dest = $this->config->get ( $role . '_dir' ) . DIRECTORY_SEPARATOR . $info->getPackage () . DIRECTORY_SEPARATOR . $dest;
						break;
						case 'php' :
						default :
					...
					$dest = preg_replace ( array ('!\\\\+!', '!/!', "!$ds2+!" ), array (DIRECTORY_SEPARATOR, DIRECTORY_SEPARATOR, DIRECTORY_SEPARATOR ), $dest );
	...
	function _doInfo2($command, $options, $params, &$obj, $installed) {
		$reg = &$this->config->getRegistry ();
		$caption = 'About ' . $obj->getChannel () . '/' . $obj->getPackage () . '-' . $obj->getVersion ();
		$data = array ('caption' => $caption, 'border' => true );
		...
		$extends = $extends ? $obj->getPackage () . ' (extends ' . $extends . ')' : $obj->getPackage ();
		if ($src = $obj->getSourcePackage ()) {
			$extends .= ' (source package ' . $src ['channel'] . '/' . $src ['package'] . ')';
		}
		$info = array ('Release Type' => $release, 'Name' => $extends, 'Channel' => $obj->getChannel (), 'Summary' => preg_replace ( '/  +/', ' ', $obj->getSummary () ), 'Description' => preg_replace ( '/  +/', ' ', $obj->getDescription () ) );
		$info ['Maintainers'] = '';
		...
		if (isset ( $deps ['required'] ['php'] ['exclude'] )) {
			...
			if (is_array ( $deps ['required'] ['php'] ['exclude'] )) {
				$deps ['required'] ['php'] ['exclude'] = implode ( ', ', $deps ['required'] ['php'] ['exclude'] );
			}
		...
		if (isset ( $deps ['required'] ['pearinstaller'] ['max'] )) {
			$info ['Required Dependencies'] .= 's ' . $deps ['required'] ['pearinstaller'] ['min'] . '-' . $deps ['required'] ['pearinstaller'] ['max'];
		} else {
			$info ['Required Dependencies'] .= ' ' . $deps ['required'] ['pearinstaller'] ['min'] . ' or newer';
		}
		if (isset ( $deps ['required'] ['pearinstaller'] ['exclude'] )) {
			...
			if (is_array ( $deps ['required'] ['pearinstaller'] ['exclude'] )) {
				$deps ['required'] ['pearinstaller'] ['exclude'] = implode ( ', ', $deps ['required'] ['pearinstaller'] ['exclude'] );
			}
			$info ['Not Compatible with'] .= "PEAR installer\n  Versions " . $deps ['required'] ['pearinstaller'] ['exclude'];
		}
			...
			if (isset ( $deps ['required'] [$index] )) {
				...
				foreach ( $deps ['required'] [$index] as $package ) {
					...
					if (isset ( $package ['max'] ) && isset ( $package ['min'] )) {
						...
						$info [$infoindex] .= " \n  Version " . $package ['min'] . ' or newer';
					} elseif (isset ( $package ['max'] )) {
						$info [$infoindex] .= " \n  Version " . $package ['max'] . ' or older';
					}
					...
					if (isset ( $package ['exclude'] )) {
						...
						$info ['Not Compatible with'] .= 'Package ' . $reg->parsedPackageNameToString ( $package, true );
						$info ['Not Compatible with'] .= "\n  Versions " . $package ['exclude'];
					}
				}
			}
