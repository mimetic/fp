...
' ), 'upgrade' => array ('summary' => 'Upgrade Package', 'function' => 'doInstall', 'shortcut' => 'up', 'options' => array ('force' => array ('shortopt' => 'f', 'doc' => 'overwrite newer installed packages' ), 'loose' => array ('shortopt' => 'l', 'doc' => 'do not check for recommended dependency version' ), 'nodeps' => array ('shortopt' => 'n', 'doc' => 'ignore dependencies, upgrade anyway' ), 'register-only' => array ('shortopt' => 'r', 'doc' => 'do not install files, only register the package as upgraded' ), 'nobuild' => array ('shortopt' => 'B', 'doc' => 'don\'t build C extensions' ), 'nocompress' => array ('shortopt' => 'Z', 'doc' => 'request uncompressed files when downloading' ), 'installroot' => array ('shortopt' => 'R', 'arg' => 'DIR', 'doc' => 'root directory used when installing files (ala PHP\'s INSTALL_ROOT), use packagingroot for RPM' ), 'packagingroot' => array ('shortopt' => 'P', 'arg' => 'DIR', 'doc' => 'root directory used when packaging files, like RPM packaging' ), 'ignore-errors' => array ('doc' => 'force install even if there were errors' ), 'alldeps' => array ('shortopt' => 'a', 'doc' => 'install all required and optional dependencies' ), 'onlyreqdeps' => array ('shortopt' => 'o', 'doc' => 'install all required dependencies' ), 'offline' => array ('shortopt' => 'O', 'doc' => 'do not attempt to download any urls or contact channels' ), 'pretend' => array ('shortopt' => 'p', 'doc' => 'Only list the packages that would be downloaded' ) ), 'doc' => '<package> ...
...
' ) );
	...
	function _parseIni($filename) {
		if (file_exists ( $filename )) {
			...
			return array ('extensions' => $extensions, 'zend_extensions' => $zend_extensions, 'extension_dir' => $extension_dir, 'all' => $all );
		} else {
		...
		if ($command == 'upgrade-all') {
			...
			foreach ( $reg->listChannels () as $channel ) {
				...
				foreach ( $latest as $package => $info ) {
					...
					$params [] = $a = $reg->parsedPackageNameToString ( array ('package' => $package, 'channel' => $channel ) );
			...
			if (is_array ( $info )) {
				if ($param->getPackageType () == 'extsrc' || $param->getPackageType () == 'extbin' || $param->getPackageType () == 'zendextsrc' || $param->getPackageType () == 'zendextbin') {
					...
					foreach ( $instpkg->getFilelist () as $name => $atts ) {
						...
						if ((strpos ( $pinfo ['basename'], 'php_' ) === 0 && $pinfo ['extension'] == 'dll') || // most unices
						$pinfo ['extension'] == 'so' || // hp-ux
						$pinfo ['extension'] == 'sl') {
				...
				if ($deps) {
					if (isset ( $deps ['group'] )) {
						...
						foreach ( $groups as $group ) {
							...
							$this->ui->outputData ( $param->getPackage () . ': Optional feature ' . $group ['attribs'] ['name'] . ' available (' . $group ['attribs'] ['hint'] . ')' );
						}
						$extrainfo [] = 'To install use "pear install ' . $reg->parsedPackageNameToString ( array ('package' => $param->getPackage (), 'channel' => $param->getChannel () ), true ) . '#featurename"';
					}
				}
		...
		foreach ( $params as $pkg ) {
			...
			if ($info === null) {
				...
				if ($installedbinary = $info->getInstalledBinary ()) {
					$this->ui->log ( 'adding binary package ' . $reg->parsedPackageNameToString ( array ('channel' => $channel, 'package' => $installedbinary ), true ) );
