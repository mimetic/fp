...
class PEAR_Command_Remote extends PEAR_Command_Common {
	...
	var $commands = array ('remote-info' => array ('summary' => 'Information About Remote Packages', 'function' => 'doRemoteInfo', 'shortcut' => 'ri', 'options' => array ( ), 'doc' => '<package>
Get details on a package from the server.' ), 'list-upgrades' => array ('summary' => 'List Available Upgrades', 'function' => 'doListUpgrades', 'shortcut' => 'lu', 'options' => array ( ), 'doc' => '[preferred_state]
...
latest stable release of each package.' ), 'search' => array ('summary' => 'Search remote package database', 'function' => 'doSearch', 'shortcut' => 'sp', 'options' => array ('channel' => array ('shortopt' => 'c', 'doc' => 'specify a channel other than the default channel', 'arg' => 'CHAN' ) ), 'doc' => '[packagename] [packageinfo]
...
will be used to match any portion of the summary/description' ), 'list-all' => array ('summary' => 'List All Packages', 'function' => 'doListAll', 'shortcut' => 'la', 'options' => array ('channel' => array ('shortopt' => 'c', 'doc' => 'specify a channel other than the default channel', 'arg' => 'CHAN' ) ), 'doc' => '
Lists the packages available on the configured server along with the
...
' ) );
	...
	function doRemoteList($command, $options, $params) {
		...
		$i = $j = 0;
		$data = array ('caption' => 'Channel ' . $channel . ' Available packages:', 'border' => true, 'headline' => array ('Package', 'Version' ) );
		if (count ( $available ) == 0) {
			$data = '(no packages available yet)';
		} else {
			foreach ( $available as $name => $info ) {
				$data ['data'] [] = array ($name, (isset ( $info ['stable'] ) && $info ['stable']) ? $info ['stable'] : '-n/a-' );
			}
		}
	...
	function doListAll($command, $options, $params) {
		...
		$data = array ('caption' => 'All packages:', 'border' => true, 'headline' => array ('Package', 'Latest', 'Local' ) );
		...
		foreach ( $available as $name => $info ) {
			...
			$data ['data'] [$info ['category']] [] = array ($reg->channelAlias ( $channel ) . '/' . $name, isset ( $info ['stable'] ) ? $info ['stable'] : null, isset ( $installed ['version'] ) ? $installed ['version'] : null, isset ( $desc ) ? $desc : null, isset ( $info ['deps'] ) ? $info ['deps'] : null );
		}
	...
	function doSearch($command, $options, $params) {
		...
		$data = array ('caption' => 'Matched packages, channel ' . $channel . ':', 'border' => true, 'headline' => array ('Package', 'Stable/(Latest)', 'Local' ) );
		...
		foreach ( $available as $name => $info ) {
			...
			$data ['data'] [$info ['category']] [] = array ($name, $info ['stable'] . $unstable, $version, $desc );
		}
		$this->ui->outputData ( $data, $command );
		$this->config->set ( 'default_channel', $channel );
		return true;
	...
	function doListUpgrades($command, $options, $params) {
		...
		foreach ( $reg->listChannels () as $channel ) {
			...
			$data = array ('caption' => $caption, 'border' => 1, 'headline' => array ('Channel', 'Package', 'Local', 'Remote', 'Size' ) );
	...
	function doClearCache($command, $options, $params) {
		...
		while ( $ent = readdir ( $dp ) ) {
			if (preg_match ( '/^xmlrpc_cache_[a-z0-9]{32}$/', $ent ) || preg_match ( '/rest.cache(file|id)$/', $ent )) {
