...
class PEAR_Installer extends PEAR_Downloader {
	...
	function _installFile2(&$pkg, $file, $atts, $tmp_path, $options) {
		...
		$this->addFileOperation ( "installed_as", array ($file, $installed_as, $save_destdir, dirname ( substr ( $dest_file, strlen ( $save_destdir ) ) ) ) );
	...
	function addFileOperation($type, $data) {
		if (! is_array ( $data )) {
			return $this->raiseError ( 'Internal Error: $data in addFileOperation' . ' must be an array, was ' . gettype ( $data ) );
		}
		if ($type == 'chmod') {
				...
				case 'delete' :
					...
					if (file_exists ( $data [0] )) {
						if (! is_writable ( dirname ( $data [0] ) )) {
							...
							$fp = false;
							if (! is_dir ( $data [0] ) && (! is_writable ( $data [0] ) || ! ($fp = @fopen ( $data [0], 'a' )))) {
								$errors [] = "permission denied ($type): $data[0]";
							} elseif ($fp) {
								fclose ( $fp );
							}
			...
			switch ( $type) {
				case 'backup' :
					...
					if (! @copy ( $data [0], $data [0] . '.bak' )) {
						$this->log ( 1, 'Could not copy ' . $data [0] . ' to ' . $data [0] . '.bak ' . $php_errormsg );
						return false;
					}
				...
				case 'rename' :
					...
					if (! $test && file_exists ( $data [1] )) {
						...
						if (! isset ( $this->_options ['soft'] )) {
							$this->log ( 1, 'Could not delete ' . $data [1] . ', cannot rename ' . $data [0] . $extra );
						}
						if (! isset ( $this->_options ['ignore-errors'] )) {
							return false;
						}
					...
					if (! @copy ( $data [0], $data [1] )) {
						$this->log ( 1, 'Could not rename ' . $data [0] . ' to ' . $data [1] . ' ' . $php_errormsg );
						return false;
					}
				...
				case 'chmod' :
					if (! @chmod ( $data [1], $data [0] )) {
						$this->log ( 1, 'Could not chmod ' . $data [1] . ' to ' . decoct ( $data [0] ) . ' ' . $php_errormsg );
						return false;
					}
					$octmode = decoct ( $data [0] );
				...
				case 'rmdir' :
					if (file_exists ( $data [0] )) {
						do {
							...
							closedir ( $testme );
							if (! @rmdir ( $data [0] )) {
								$this->log ( 1, 'Could not rmdir ' . $data [0] . ' ' . $php_errormsg );
								return false;
							}
	...
	function _compileSourceFiles($savechannel, &$filelist) {
		...
		foreach ( $built as $ext ) {
			...
			if ($filelist->getPackageXmlVersion () == '1.0') {
				$filelist->installedFile ( $bn, array ('role' => $role, 'name' => $bn, 'installed_as' => $dest, 'php_api' => $ext ['php_api'], 'zend_mod_api' => $ext ['zend_mod_api'], 'zend_ext_api' => $ext ['zend_ext_api'] ) );
			} else {
				$filelist->installedFile ( $bn, array ('attribs' => array ('role' => $role, 'name' => $bn, 'installed_as' => $dest, 'php_api' => $ext ['php_api'], 'zend_mod_api' => $ext ['zend_mod_api'], 'zend_ext_api' => $ext ['zend_ext_api'] ) ) );
			}
		}
	}
