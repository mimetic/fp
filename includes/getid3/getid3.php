...
class getID3
{
	...
	function getID3()
	{
		...
		if (preg_match_all('/([0-9]+)M/i', $memory_limit, $matches)) {
			// could be stored as "16M" rather than 16777216 for example
			$matches = $matches[1];
			if ($matches)
				$memory_limit = $matches[0] * 1048576;
		}
		if ($memory_limit <= 0) {
			...
			if ((($this->info['filesize'] != 0) && (ftell($fp) == 0)) ||
				...
				(ftell($fp) < 0)) {
					...
					if (GETID3_OS_ISWINDOWS) {
						...
						if (preg_match_all('/1 File\(s\)[ ]+([0-9]+) bytes/i', $dir_output, $matches)) {
							$matches = $matches[1];
							$real_filesize = (float) $matches[0];
						}
					} else {
						$commandline = 'ls -o -g -G --time-style=long-iso '.escapeshellarg($filename);
						$dir_output = `$commandline`;
						if (preg_match_all('/([0-9]+) ([0-9]{4}-[0-9]{2}\-[0-9]{2} [0-9]{2}:[0-9]{2}) '.preg_quote($filename).'$/i', $dir_output, $matches)) {
							$matches = $matches[1];
							$real_filesize = (float) $matches[0];
						}
					}
