...
class SlideshowXML {
	...
	//	header("Content-type: application/XML+xml");
		...
		$DEBUG = false;
		if ($sspversion == "1.5x")	
			$DEBUG && file_put_contents ("xmloutput.html", $output);
		if ($sspversion == "1.6x")	
	...
	function outputalbums() {
		...
		foreach ($this->albums as $album) {
			...
			" maxImageWidth=\"" . $album['maxImageWidth'].'"' .
			" maxImageHeight=\"" . $album['maxImageHeight'].'"' .
			" textColor=\"" . $album['textColor'].'"' .
			" frameColor=\"" . $album['frameColor'].'"' .
			" frameWidth=\"" . $album['frameWidth'].'"' .
			" stagePadding=\"" . $album['stagePadding'].'"' .
			" navPadding=\"" . $album['navPadding'].'"' .
			" thumbnailColumns=\"" . $album['thumbnailColumns'].'"' .
			" thumbnailRows=\"" . $album['thumbnailRows'].'"' .
			" vAlign=\"" . $album['vAlign'].'"' .
			" hAlign=\"" . $album['hAlign'].'"' .
//			" description=\"" . $album['description'] . '"' .
			...
			foreach ($album['images'] as $image) {
				...
				if ($SlideShowAbsPaths) {
					...
					$output .=  "<image>\n";
					$output .=  "\t<filename>" . htmlspecialchars($filename) . "</filename>\n";
